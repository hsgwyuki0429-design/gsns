...
- 縦スワイプのゲームフィード(スクロールスナップ)。プレイ中もゆっくりしたドラッグで移動可。PCはマウスホイール / ↑↓キー / 画面右のボタンで移動
- 読み込めないゲーム(ファイル消失・スクリプトのクラッシュ等)はエラー表示になり、タップで再試行・スワイプでスキップ可能
...
- 効果音は WebAudio で合成する(外部音声ファイルは読み込めません)。効果音の生成に `Math.random()` を使わないこと(乱数列がズレてリプレイが壊れます)。リプレイ時はブラウザの自動再生制限により無音になることがあります
- 画面上部約60pxはアプリのUIと重なるためHUDは避ける
- **ゲームが終了したら `parent.postMessage({gsns:'game-over'},'*')` を送る**。プレイの記録がその時点で確定・保存され、プレイヤーに「上にスワイプで次へ」のヒントが表示されてフィード操作が戻ります(送らない場合も、素早い縦フリックでプレイ中にスキップできます)
